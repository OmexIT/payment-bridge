...
import com.omexit.paymentbridge.dto.RestError;
...
public abstract class BaseController {
	...
	public ResponseEntity<Exception> handleConstraintViolationException(ConstraintViolationException cre) {
	...
	/**
	 * Handles HttpMessageNotReadableException Exceptions not addressed by more
	 * specific <code>@ExceptionHandler</code> methods. Creates a response with the
	 * Exception detail in the response body as JSON and a HTTP status code of 500,
	...
	public ResponseEntity<RestError> handleHttpMessageNotReadableException(HttpMessageNotReadableException e) {
		...
		// logger.error("< handleException");
		RestError error = new RestError(HttpStatus.BAD_REQUEST.value(), 1002, "Request not readable", e.getMessage(),
				e.getMessage());
	...
	public ResponseEntity<RestError> handleRetailerNotFoundException(ResourceNotFoundException e) {
		...
		RestError error = new RestError(HttpStatus.NOT_FOUND.value(), 1002, "Resource not found", e.getMessage(),
				e.getMessage());
