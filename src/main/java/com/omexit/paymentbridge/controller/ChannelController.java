package com.omexit.paymentbridge.controller;
...
import com.omexit.paymentbridge.exception.ResourceNotFoundException;
...
public class ChannelController extends BaseController {
	...
	/**
	 * Web service end point to fetch all Channel entities. The service returns the
	 * collection of Channel entity in JSON
	 *
	...
	/**
	 * Web service endpoint to fetch a single Channel entity by primary key
	 * identifier
	 * <p>
	...
	/**
	 * Web service endpoint to create a single Channel entity. The HTTP request body
	 * is expected to contain a Channel object in JSON format. The Channel is
	 * persisted in the data repository.
	 * <p>
	 * If created successfully, the persisted Channel is returned as JSON with HTTP
	 * status 201.
	 * <p>
	 * If not created successfully, the service returns an empty response body with
	 * HTTP status 500.
	 *
	 * @param channel
	 *            The Channel object to be created.
	 ...
	 *         comment.
	 * @throws Exception
	 *             Thrown if a problem occurs completing the request.
	 */
	@RequestMapping(value = "/channel", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	...
	/**
	 * Web service endpoint to update a single Channel entity. The HTTP request body
	 * is expected to contain a Channel object in JSON format. The Channel is
	 * updated in the data repository.
	 * <p>
	 * If updated successfully, the persisted Channel is returned as JSON with HTTP
	 * status 200.
	 * <p>
	 * If not found, the service returns an empty response body and HTTP status 404.
	 * <p>
	 * If not updated successfully, the service returns an empty response body with
	 * HTTP status 500.
	 *
	 * @param channel
	 *            The Channel object to be updated.
	 ...
	 *             Thrown if a problem occurs completing the request.
	 */
	@RequestMapping(value = "/channel/{id}", method = RequestMethod.PUT, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
